...
(function (global) {
  ...
  // ================================================================
  if (Game.meditate && !Game.__v435remake_meditate) {
  ...
  // ================================================================
  // 十、【炼丹房 & 灵植园】简化的可用面板（避免上一版没挂到 UI）
  // ================================================================
  Game.openAlchemyPanel = function () {
    // [v4.3.5e-fix] state 未就绪时尝试先 init，仍失败就提示而不崩溃
    if (!Game.state || Game.state.__placeholder) {
    ...
    Game.state.inventory_v435 = Game.state.inventory_v435 || { pills:{}, herbs:{} };
    const c = Game.state.char;
    const inv = Game.state.inventory_v435;
    const curRealmId = c.realmId || 1;
    const availPills = PILLS.filter(p => curRealmId + 2 >= p.realmId);
    const html = `
      <div class="v435-scene-alchemy" style="padding:20px;border-radius:12px;min-height:400px;">
        <h2 style="color:#f5c97a;text-align:center;letter-spacing:4px;margin:0 0 8px;">⚗ 丹鼎峰·炼丹房</h2>
        <div style="text-align:center;color:#e9d8b6;font-size:12px;margin-bottom:16px;">💰 灵石：${c.lingshi || 0} · 你的境界：${(REALMS[curRealmId] && REALMS[curRealmId].name) || '凡人'}</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px;">
        ${availPills.map(p => `
          <div style="padding:12px;background:rgba(20,18,42,0.85);border:1px solid #f5c97a55;border-radius:10px;">
            <div style="color:#f5c97a;font-weight:700;">${p.name} ${p.realmId > curRealmId ? '<span style="font-size:11px;color:#e25b5b;">·需高阶</span>' : ''}</div>
            <div style="font-size:11px;color:#b288ff;margin:4px 0;">${p.effect}</div>
            <div style="font-size:11px;color:#8ce28c;">需灵石×${p.cost}</div>
            <div style="font-size:10px;color:#888;margin:4px 0;">草药：${p.herbs.map(h => (HERBS.find(hh=>hh.id===h)||{}).name || h).join('、')}</div>
            <div style="font-size:11px;color:#f5c97a;">拥有：${(inv.pills[p.id]||0)}颗</div>
            <button ${(c.lingshi||0) < p.cost || p.realmId > curRealmId ? 'disabled' : ''}
              onclick="Game._buyPill('${p.id}')" style="width:100%;margin-top:6px;padding:5px;background:${(c.lingshi||0) < p.cost || p.realmId > curRealmId ? '#555' : '#f5c97a'};color:#20182a;border:none;border-radius:4px;font-weight:700;cursor:${(c.lingshi||0) < p.cost || p.realmId > curRealmId ? 'not-allowed' : 'pointer'};">
              🔥 炼制（${p.cost} 灵石）
            </button>
          </div>
        `).join('')}
    ...
    `;
    if (UI && UI.showModal) {
      UI.showModal({ title: '⚗ 炼丹房', body: html, width: '760px' });
    } else {
      alert('炼丹房：需要 UI.showModal 支持');
    }
  };
  Game._buyPill = function (id) {
    const p = PILLS.find(x => x.id === id);
    if (!p) return;
    const c = Game.state.char;
    if (c.lingshi < p.cost) { UI.toast('灵石不足', 'error'); return; }
    c.lingshi -= p.cost;
    Game.addPill(id, 1);
    UI.toast(`🎉 炼制成功！获得 ${p.name} ×1`, 'success');
    // 刷新面板
    if (document.querySelector('.v435-scene-alchemy')) {
      document.querySelector('.v435-scene-alchemy').closest('.xx-modal-body') &&
        (document.querySelector('.v435-scene-alchemy').closest('.xx-modal-body').innerHTML = '');
      Game.openAlchemyPanel();
    }
  };
  ...
  Game.openGardenPanel = function () {
    ...
    const inv = Game.state.inventory_v435;
    const curRealmId = c.realmId || 1;
    const availHerbs = HERBS.filter(h => curRealmId + 2 >= h.realmId);
    const html = `
      <div class="v435-scene-garden" style="padding:20px;border-radius:12px;min-height:400px;">
        <h2 style="color:#8ce28c;text-align:center;letter-spacing:4px;margin:0 0 8px;">🌱 灵植园</h2>
        <div style="text-align:center;color:#e9d8b6;font-size:12px;margin-bottom:16px;">💰 灵石：${c.lingshi || 0}</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px;">
        ${availHerbs.map(h => {
          const cost = 5 + h.realmId * 3;
          return `
            <div style="padding:12px;background:rgba(20,18,42,0.85);border:1px solid #8ce28c55;border-radius:10px;">
              <div style="color:#8ce28c;font-weight:700;">${h.name}</div>
              <div style="font-size:11px;color:#e9d8b6;margin:4px 0;">${h.desc}</div>
              <div style="font-size:11px;color:#f5c97a;">库存：${(inv.herbs[h.id]||0)}株</div>
              <button ${c.lingshi < cost ? 'disabled' : ''}
                onclick="Game._buyHerb('${h.id}', ${cost})" style="width:100%;margin-top:6px;padding:5px;background:${c.lingshi < cost ? '#555' : '#8ce28c'};color:#20182a;border:none;border-radius:4px;font-weight:700;cursor:${c.lingshi < cost ? 'not-allowed' : 'pointer'};">
                🌱 采集（${cost} 灵石）
              </button>
            </div>
          `;
    ...
    `;
    if (UI && UI.showModal) UI.showModal({ title: '🌱 灵植园', body: html, width: '760px' });
  };
  Game._buyHerb = function (id, cost) {
    const c = Game.state.char;
    if (c.lingshi < cost) { UI.toast('灵石不足', 'error'); return; }
    c.lingshi -= cost;
    Game.addHerb(id, 1);
    UI.toast(`🌱 采集到 ${(HERBS.find(x=>x.id===id)||{}).name} ×1`, 'success');
    Game.openGardenPanel();
  };
  ...
  Game.openSpellPanel = function (sectKey) {
    ...
    `;
    if (UI && UI.showModal) UI.showModal({ title: '✦ 法术真解', body: html, width: '760px' });
  };
