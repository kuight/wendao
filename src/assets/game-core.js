...
(function (global) {
  ...
  const Game = {
    ...
    // ==========================================================
    // v4.3.5g-r2 重写：存档自动矫正 + 属性逐层重算
    //
    // 关键变化：
    //   1、锁改为按 realmId — 每次境界变化都重算（旧存档一进来就矫正）
    //   2、属性成长曲线加强：
    //      旧：addHp += 15*(1+l*0.02) → rid100 时 addHp≈3000 太少
    //      新：addHp += 20*(1+l*0.03) 再乘境界校准 → rid100 时 ≈ 6万
    //   3、HP/MP 当前值按旧百分比保留（避免读档满血或空血）
    //   4、附加：首次读档时静默追认已达标成就（防止 Bug 3 弹 13 项 toast）
    // ==========================================================
    _catchUpStats() {
      const c = this.state.char;
      if (!c) return;
      const id = c.realmId || 0;
      // 按 realmId 锁：同一境界不重复追赶（同时兼容旧标记）
      if (c._v435gStatsCatchupRid === id && c._v435gStatsCatchup) {
        // 但静默成就追认只做一次
      ...
      for (let l = 1; l <= id; l++) {
        const inc = 1 + l * 0.03; // 0.02 → 0.03
        addHp += Math.round(20 * inc);
        addMp += Math.round(10 * inc);
        addAtk += Math.max(2, Math.round(4 * inc));
        addDef += Math.max(1, Math.round(2 * inc));
        if (l % 8 === 0) addCrit += 0.01;
      }

      // 境界校准乘子：让玩家属性跟得上怪物曲线
      const realmMul = (this.getRealmMultiplier ? this.getRealmMultiplier() : 1);
      const scale = 1 + (realmMul - 1) * 0.35;
      ...
      // 保留 HP/MP 百分比
      const oldHp = c.hp || targetMaxHp, oldMaxHp = c.maxHp || targetMaxHp;
      const oldMp = c.mp || targetMaxMp, oldMaxMp = c.maxMp || targetMaxMp;
      ...
      c.def = targetDef;
      c.crit = targetCrit;
      c.hp = Math.max(1, Math.round(c.maxHp * hpPct));
      c.mp = Math.max(0, Math.round(c.maxMp * mpPct));

      c._v435gStatsCatchup = true;
      c._v435gStatsCatchupRid = id;
      c._v4StatsCatchup = true; // 兼容旧锁

      console.info(`[Game v4.3.5g-r2] 属性矫正：境界层 ${id} realmMul=×${realmMul.toFixed(2)} → HP:${c.maxHp} MP:${c.maxMp} ATK:${c.atk} DEF:${c.def} CRIT:${(c.crit*100).toFixed(0)}%`);
    ...
    // v4.3.5g-r2：首次读档时静默将已达标的成就标为 done（不弹 toast、不发奖励）
    _silentAuditAchievements() {
      ...
      if (audited > 0) console.info(`[Game v4.3.5g-r2] 静默追认 ${audited} 项已达标成就（不弹 toast）`);
    },
    ...
    getRealmMultiplier(rid) {
      const id = (typeof rid === 'number' && rid >= 0) ? rid : (this.state.char.realmId || 0);
      if (id <= 0) return 1;
      // v4.3.5g-r2 平缓化：金丹初期 ~326 题突破，金丹中 ~491 题（原 55 题太快）
    ...
    // v4.3.5i 直接内置：目标属性（供指数曲线怪物 / 玩家追赶用）
    //   之前来自 v435c-hotfix；现在放进核心，避免删了补丁就失效
    getTargetStatsForRealm(realmId) {
      const id = Math.max(0, realmId || 0);
      if (id <= 0) return { maxHp:100, maxMp:50, atk:20, def:5 };
      const rm = Math.max(1, this.getRealmMultiplier(id));
      return {
    ...
    //   之前定义在 _archived_patches/v435f-hotfix.js 里但从未加载
    //   算法：以传入 enemyRid 的目标属性为基准；难度乘法；越阶再叠加
    scaleMonsterForRealm(base) {
      ...
      const hpVal  = Math.max(80, Math.round(target.maxHp * 0.88 * diffMul * gapHp));
      const atkVal = Math.max(8,  Math.round(target.atk   * 0.62 * diffMul * gapAtk));
      const defVal = Math.max(1,  Math.round((target.def || 0) * 0.42 * diffMul));
    ...
    doRefine(sect, mid) {
      ...
      // v4.3.5g-r2 Bug 1 修复：推进每日任务中的"精进"类型（之前从未推进）
    ...
    meditate() {
      ...
      // v4.3.5g-r2：洞府奖励 ÷10（60→6, 1.35→1.18 使指数增速也变缓）
      const caveExpMul = Math.pow(1.18, caveLv);
      const expGain  = Math.round(6 * caveExpMul * realmMul);
      const daoGain  = Math.round(4 + realmMul * 0.5);
      const healHp   = Math.round(10 * realmMul);
      const healMp   = Math.round(6  * realmMul);
    ...
    enterSecret() {
      ...
      const events = [
        ...
        { id:'pill',   weight:30, msg:'你在洞中拾得一枚培元丹！', effect:()=>{ const rm=self.getRealmMultiplier(); const g=Math.round(50*rm); self.changeDaoxin(5); self.addExp(g); self.addPill('pill_peiyuan', 1); return `道心+5, 修为+${g.toLocaleString()}, 培元丹×1`; } },
        { id:'gem',    weight:25, msg:'墙壁中嵌有一颗灵晶矿！',   effect:()=>{ const rm=self.getRealmMultiplier(); const n = Math.round((70+Math.floor(Math.random()*90))*rm); self.addLingshi(n); return `灵石+${n.toLocaleString()}`; } },
        { id:'gear',   weight:15, msg:'你发现一件古战场遗物！',   effect:()=>{ const pool = ['weapon_iron_sword','robe_qingpao','focus_yushu','crown_muguan']; const gid = pool[Math.floor(Math.random()*pool.length)]; self.addGear(gid); const rm=self.getRealmMultiplier(); const bonusShi=Math.round(30*rm); self.addLingshi(bonusShi); return `获得装备：${(Game.GEAR_DATA[gid]||{}).name || gid}，另得灵石+${bonusShi.toLocaleString()}`; } },
        { id:'senior', weight:15, msg:'你遇到一位前辈，他授你一诀……', effect:()=>{ const rm=self.getRealmMultiplier(); const g=Math.round(120*rm); self.addExp(g); return `获前辈赞赏，修为+${g.toLocaleString()}`; } },
        { id:'demon',  weight:10, msg:'秘境深处遇到妖兽袭击！',   effect:()=>{ const rm=self.getRealmMultiplier(); self.changeDaoxin(-8); const shi=Math.round(12*rm); self.addLingshi(shi); return `惊险逃脱，道心-8，但拾得妖兽遗物 灵石+${shi.toLocaleString()}`; } },
        { id:'manual', weight:5,  msg:'你在残卷中悟得一缕大道！', effect:()=>{ const rm=self.getRealmMultiplier(); const g=Math.round(350*rm); self.addExp(g); return `修为+${g.toLocaleString()}！`; } },
        { id:'lucky',  weight:3,  msg:'机缘巧合，元气暴涨！',     effect:()=>{ const rm=self.getRealmMultiplier(); const n = Math.round((200+Math.floor(Math.random()*200))*rm); const e = Math.round(150*rm); self.addLingshi(n); self.addExp(e); return `灵石+${n.toLocaleString()}, 修为+${e.toLocaleString()}`; } },
        // v4.2.6 新增：高境界小概率奇遇
        { id:'heavenly', weight:2, msg:'天花乱坠！你感惟多年古道之響！', effect:()=>{ const rm=self.getRealmMultiplier(); const e=Math.round(1500*rm); self.addExp(e); self.changeDaoxin(6); return `天花坠兽，道心+6，修为+${e.toLocaleString()}`; } },
      ];
